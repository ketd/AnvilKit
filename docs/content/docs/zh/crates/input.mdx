---
title: anvilkit-input
description: 输入系统 — 键盘、鼠标、Action Map、RenderApp 集成
---

`anvilkit-input` 提供跨平台的输入抽象，支持键盘和鼠标输入、逻辑 Action Map 系统，以及与渲染窗口事件循环集成的辅助方法。

## InputState

`InputState` 是一个 ECS `Resource`，追踪每帧的键盘和鼠标状态。它区分持续按下、首帧按下和首帧松开。

### 键盘 API

| 方法 | 签名 | 说明 |
|------|------|------|
| `press_key` | `(&mut self, KeyCode)` | 记录按键按下。如果该键之前未被按住，同时标记为"刚按下"。 |
| `release_key` | `(&mut self, KeyCode)` | 记录按键松开。如果该键之前被按住，标记为"刚松开"。 |
| `is_key_pressed` | `(&self, KeyCode) -> bool` | 按键被按住时为 true（跨帧持续）。 |
| `is_key_just_pressed` | `(&self, KeyCode) -> bool` | 仅在按键首次按下的那一帧为 true。 |
| `is_key_just_released` | `(&self, KeyCode) -> bool` | 仅在按键松开的那一帧为 true。 |
| `pressed_keys` | `(&self) -> &HashSet<KeyCode>` | 当前所有被按住的键。 |

### 鼠标按钮 API

| 方法 | 签名 | 说明 |
|------|------|------|
| `press_mouse` | `(&mut self, MouseButton)` | 记录鼠标按钮按下。 |
| `release_mouse` | `(&mut self, MouseButton)` | 记录鼠标按钮松开。 |
| `is_mouse_pressed` | `(&self, MouseButton) -> bool` | 按钮被按住时为 true。 |
| `is_mouse_just_pressed` | `(&self, MouseButton) -> bool` | 仅在按下帧为 true。 |
| `is_mouse_just_released` | `(&self, MouseButton) -> bool` | 仅在松开帧为 true。 |

`MouseButton` 变体：`Left`、`Right`、`Middle`。

### 鼠标位置 / 移动 API

| 方法 | 签名 | 说明 |
|------|------|------|
| `set_mouse_position` | `(&mut self, Vec2)` | 设置光标绝对位置（像素）。 |
| `mouse_position` | `(&self) -> Vec2` | 获取当前光标位置。 |
| `add_mouse_delta` | `(&mut self, Vec2)` | 累加原始鼠标移动增量。 |
| `mouse_delta` | `(&self) -> Vec2` | 获取本帧累计移动增量（像素）。 |
| `add_scroll_delta` | `(&mut self, f32)` | 累加滚轮增量（行数）。 |
| `scroll_delta` | `(&self) -> f32` | 获取本帧累计滚轮增量。 |

### 帧生命周期

| 方法 | 签名 | 说明 |
|------|------|------|
| `end_frame` | `(&mut self)` | 清除所有 `just_pressed`、`just_released`、`mouse_delta` 和 `scroll_delta`。每帧结束时调用一次。 |

### 使用示例

```rust
use anvilkit_input::input_state::{InputState, KeyCode, MouseButton};

fn player_system(input: Res<InputState>) {
    // 移动（持续按住）
    if input.is_key_pressed(KeyCode::W) {
        // 向前移动
    }
    if input.is_key_pressed(KeyCode::S) {
        // 向后移动
    }

    // 跳跃（刚按下——只触发一次）
    if input.is_key_just_pressed(KeyCode::Space) {
        // 跳跃
    }

    // 射击
    if input.is_mouse_pressed(MouseButton::Left) {
        // 开火
    }

    // 摄像机视角（FPS 原始增量）
    let delta = input.mouse_delta();
    camera_yaw   -= delta.x * sensitivity;
    camera_pitch -= delta.y * sensitivity;

    // 缩放
    let scroll = input.scroll_delta();
    fov -= scroll * 2.0;
}
```

## ActionMap

`ActionMap` 将逻辑动作名（字符串）映射到一个或多个物理 `InputBinding`。任一绑定激活即视为动作激活。

### InputBinding

```rust
pub enum InputBinding {
    Key(KeyCode),
    Mouse(MouseButton),
}
```

### ActionState

| 变体 | `is_active()` | `is_just_pressed()` | `is_just_released()` |
|------|---------------|----------------------|----------------------|
| `Inactive` | false | false | false |
| `JustPressed` | true | true | false |
| `Pressed` | true | false | false |
| `JustReleased` | false | false | true |

### ActionMap API

| 方法 | 签名 | 说明 |
|------|------|------|
| `new` | `() -> Self` | 创建空的动作映射表。 |
| `add_binding` | `(&mut self, action: &str, InputBinding)` | 为命名动作绑定一个输入。每个动作支持多个绑定。 |
| `update` | `(&mut self, &InputState)` | 根据当前输入重新计算所有动作状态。每帧在输入事件之后调用一次。 |
| `action_state` | `(&self, action: &str) -> ActionState` | 获取动作的完整状态。未知动作返回 `Inactive`。 |
| `is_action_active` | `(&self, action: &str) -> bool` | `JustPressed` 或 `Pressed` 时为 true。 |
| `is_action_just_pressed` | `(&self, action: &str) -> bool` | 仅在激活帧为 true。 |
| `is_action_just_released` | `(&self, action: &str) -> bool` | 仅在停用帧为 true。 |
| `get_bindings` | `(&self, action: &str) -> Option<&[InputBinding]>` | 查看当前绑定。 |
| `clear_bindings` | `(&mut self, action: &str)` | 移除某动作的所有绑定。 |

### 使用示例

```rust
use anvilkit_input::prelude::*;
use anvilkit_input::action_map::InputBinding;

// 设置（通常在 Startup 中）
fn setup_actions(mut commands: Commands) {
    let mut map = ActionMap::new();

    map.add_binding("move_forward", InputBinding::Key(KeyCode::W));
    map.add_binding("move_forward", InputBinding::Key(KeyCode::Up));
    map.add_binding("move_back",    InputBinding::Key(KeyCode::S));
    map.add_binding("move_back",    InputBinding::Key(KeyCode::Down));
    map.add_binding("jump",         InputBinding::Key(KeyCode::Space));
    map.add_binding("shoot",        InputBinding::Mouse(MouseButton::Left));
    map.add_binding("aim",          InputBinding::Mouse(MouseButton::Right));

    commands.insert_resource(map);
}

// 每帧更新（PreUpdate）
fn update_actions(input: Res<InputState>, mut actions: ResMut<ActionMap>) {
    actions.update(&input);
}

// 游戏逻辑（Update）
fn game_logic(actions: Res<ActionMap>) {
    if actions.is_action_active("move_forward") {
        // 向前移动
    }
    if actions.is_action_just_pressed("jump") {
        // 开始跳跃
    }
    if actions.is_action_just_released("shoot") {
        // 释放蓄力射击
    }
}
```

## RenderApp 集成

使用 `anvilkit-render` 的 `RenderApp` 时，三个辅助方法自动将 OS 窗口事件桥接到 ECS `InputState` 资源。自定义 `ApplicationHandler` 的游戏可以直接调用这些方法。

| 方法 | 类型 | 签名 | 说明 |
|------|------|------|------|
| `forward_input` | 静态 | `(app: &mut App, event: &WindowEvent)` | 将键盘、鼠标按钮、光标位置和滚轮事件转换为 `InputState` 调用。 |
| `forward_device_input` | 静态 | `(app: &mut App, event: &DeviceEvent)` | 将原始 `DeviceEvent::MouseMotion` 转换为 `InputState::add_mouse_delta`。用于 FPS 风格的摄像机控制。 |
| `tick` | 实例 | `(&mut self, app: &mut App)` | 运行一帧：更新 `DeltaTime`，调用 `app.update()`，然后调用 `InputState::end_frame()`。 |

### 典型自定义事件循环

```rust
use anvilkit_render::RenderApp;
use winit::event::{WindowEvent, DeviceEvent};

// 在 ApplicationHandler 实现中：

fn window_event(&mut self, event_loop, window_id, event: WindowEvent) {
    // 让引擎处理输入记录
    RenderApp::forward_input(&mut self.app, &event);

    // 处理游戏特定的窗口事件
    match &event {
        WindowEvent::CloseRequested => event_loop.exit(),
        _ => {}
    }
}

fn device_event(&mut self, event_loop, device_id, event: DeviceEvent) {
    // FPS 摄像机的原始鼠标移动
    RenderApp::forward_device_input(&mut self.app, &event);
}

fn about_to_wait(&mut self, event_loop) {
    // 标准每帧生命周期：DeltaTime → app.update() → end_frame()
    self.render_app.tick(&mut self.app);
    self.window.request_redraw();
}
```

## 鼠标锁定

FPS 游戏通常锁定光标并使用原始增量：

```rust
use winit::window::CursorGrabMode;

// 锁定光标到窗口
window.set_cursor_grab(CursorGrabMode::Confined)
    .or_else(|_| window.set_cursor_grab(CursorGrabMode::Locked))
    .unwrap();
window.set_cursor_visible(false);

// 使用 DeviceEvent::MouseMotion（由 forward_device_input 转发）
// 而不是 CursorMoved，后者会在窗口边界处截断。
```
