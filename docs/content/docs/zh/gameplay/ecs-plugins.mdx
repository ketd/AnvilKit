...
`DefaultPlugins` 包含：`AnvilKitEcsPlugin` + `RenderPlugin` + `AudioPlugin` + `InputPlugin`（`InputState` + `ActionMap` + `action_map_update_system`）+ `CameraPlugin` + `AutoDeltaTimePlugin` + `PersistencePlugin`（cfg）+ `SettingsApplyPlugin`（cfg）。

此外，`anvilkit-gameplay` crate 提供了更高层的 Gameplay 插件。详见 [Gameplay 系统](/docs/zh/gameplay/gameplay-systems) 页面。
