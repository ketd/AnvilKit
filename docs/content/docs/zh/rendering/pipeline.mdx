...
- **`events.rs`**（1414 行）拆分为 `events/` 目录：`mod.rs`、`lighting.rs`、`render_app.rs`、`gpu_init.rs`、`render_loop.rs`、`input.rs`
- **`draw.rs`**（568 行）拆分为 `draw/` 目录：`mod.rs`、`culling.rs`、`lighting.rs`、`commands.rs`、`gpu.rs`
...
- **`DebugRenderer`** 现在包含了 `LineRenderer` 功能（两者在 v0.3 中合并；参见 [调试渲染器](/docs/zh/devtools/debug-renderer)）。
