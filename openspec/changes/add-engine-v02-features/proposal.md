...
- **持久化系统** — 存档管理（槽位/自动存档/版本迁移）、玩家设置持久化、大规模世界存储（KV backend）、资源编译缓存
...
- New specs: `render-post-processing`, `render-advanced`, `scene-serialization`, `persistence`, `physics-runtime`, `ui-framework`, `asset-pipeline`, `ai-navigation`, `networking`, `dev-tools`
