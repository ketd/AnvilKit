...
The facade crate SHALL re-export all commonly-used types in its prelude, including audio components (`AudioSource`, `AudioListener`), persistence types (`SaveManager`, `WorldStorage`), camera types, and `egui` core types (`egui::Context`, `egui::Ui`).
...
- **THEN** all commonly-used engine types including `egui::Context` are available without importing individual sub-crates
