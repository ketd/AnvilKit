...
### Phase A: 渲染品质 (M6b → M6d) — PBR 渲染视觉对齐 Unity URP Lit Shader

- **M6b**: 法线贴图 — MeshVertex 扩展 tangent 属性, TBN 矩阵构建, normal map 采样, glTF tangent 提取
- **M6c**: HDR 渲染管线 — Rgba16Float offscreen RT, ACES Filmic tone mapping, multi-pass 渲染 (scene pass → post-process pass)
- **M6d**: IBL 环境光 — HDR equirectangular → cubemap, diffuse irradiance 卷积, prefiltered specular (split-sum), BRDF LUT

### Phase B: 生产渲染 (M7) — 能渲染"像样的"游戏场景

- **M7a**: 多光源 — 点光源 + 聚光灯, light uniform 数组 (最多 8 盏), shader 光源循环
- **M7b**: 阴影系统 — 方向光 Cascaded Shadow Maps + shadow pass + PCF 软阴影
- **M7c**: 完整材质系统 — Metallic-Roughness 纹理 (合并通道), AO 贴图, Emissive 贴图+因子, Alpha Blend/透明队列
- **M7d**: 抗锯齿 — MSAA 4x (surface multisample)

### Phase C: 渲染性能 (M8) — 能处理 100+ 物体的场景

- **M8a**: Frustum Culling — AABB 包围盒, 视锥体剔除, 不可见物体跳过绘制
- **M8b**: GPU Instancing + 批处理 — instance buffer, 相同 mesh/material 合批, 减少 draw call
