...
## Purpose
**AnvilKit** is the first AI-agent-native game engine — a Rust game engine where AI agents are first-class users.

Core design principles:
- **AI-first** — Self-describing APIs (`Describe` trait), MCP server for agent interaction, structured errors with fix hints
- **Minimal surface** — Code-first, no GUI editor. Advanced render effects feature-gated. Fewer decisions for the agent
- **Built on bevy** — Uses `bevy_app` + `bevy_ecs` 0.15 directly. No custom ECS reimplementation
- **Modular** — Cargo feature flags for opt-in capabilities (`mcp`, `advanced-render`)

GitHub: https://github.com/ketd/AnvilKit

## Tech Stack
- **Language**: Rust (Edition 2021)
- **ECS**: `bevy_ecs 0.15` + `bevy_app 0.15` — 高性能实体组件系统
...
├── POSITIONING.md          # AI-first engine positioning
├── crates/
│   ├── anvilkit-core/      # 核心类型、数学、时间、持久化、结构化错误
│   ├── anvilkit-ecs/       # bevy_app::App re-export + ECS wrappers + physics/navigation/scene
│   ├── anvilkit-render/    # wgpu 渲染引擎 (PBR/sprites/shadow/bloom; advanced-render feature)
│   ├── anvilkit-assets/    # 资源系统 (异步加载/热重载/缓存)
│   ├── anvilkit-audio/     # 音频引擎 (rodio 集成)
│   ├── anvilkit-input/     # 跨平台输入系统 (键鼠/手柄/ActionMap)
│   ├── anvilkit-app/       # App Runner (GameCallbacks + 事件循环 + egui)
│   ├── anvilkit-describe/  # 🔥 AI: Describe trait + derive macro (self-describing types)
│   ├── anvilkit-describe-derive/ # Proc macro for #[derive(Describe)]
│   ├── anvilkit-mcp/       # 🔥 AI: MCP server (JSON-RPC stdio, agent tool interface)
│   └── anvilkit/           # 主 crate (prelude + DefaultPlugins)
├── games/
│   ├── craft/              # Minecraft 风格体素游戏 (depends on camera/gameplay/data directly)
│   └── billiards/          # PBR 台球游戏
└── examples/               # 示例项目 (game.rs, demo.rs, showcase.rs)
```

**Demoted crates** (still in tree, not in umbrella): `anvilkit-camera`, `anvilkit-gameplay`, `anvilkit-data`, `anvilkit-ui`
**Suspended**: `tools/anvilkit-cli` (not in workspace members)
