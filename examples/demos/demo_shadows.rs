//! # Demo Shadows — 级联阴影贴图演示
//!
//! 强方向光 + 单点光源，用于演示级联阴影贴图效果。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_shadows -- \
//!     --capture-dir /tmp/frames/shadows --capture-frames 120
//! ```
...
fn main() {
    ...
    app.add_plugins(RenderPlugin::new().with_window_config(
        WindowConfig::new()
            .with_title("Demo: Cascade Shadows")
            .with_size(640, 480),
    ));
    app.insert_resource(FrameTime(Instant::now()));
    app.insert_resource(SceneLights {
        directional: DirectionalLight {
            direction: glam::Vec3::new(-0.3, -0.5, 0.8).normalize(),
            color: glam::Vec3::new(1.0, 0.9, 0.7),
            intensity: 8.0,
        },
        point_lights: vec![
            PointLight { position: glam::Vec3::new(-3.0, 2.0, -3.0), color: glam::Vec3::new(0.8, 0.8, 1.0), intensity: 5.0, range: 10.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: Cascade Shadows").with_size(640, 480);

    event_loop.run_app(&mut DemoShadowsApp {
...
impl DemoShadowsApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo shadows initialized (capture: {})", self.capture.recording);
    }
    ...
    fn render_frame(&mut self) {
        ...
        for (i, cmd) in dl.commands.iter().enumerate() {
            ...
            {
                let cl = if i == 0 { wgpu::LoadOp::Clear(wgpu::Color { r: 0.05, g: 0.05, b: 0.08, a: 1.0 }) } else { wgpu::LoadOp::Load };
...
impl ApplicationHandler for DemoShadowsApp {
