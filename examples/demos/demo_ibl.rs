//! # Demo IBL — 基于图像的环境光照演示
//!
//! 展示 IBL ambient lighting：极暗方向光，让 BRDF LUT 环境光主导。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_ibl -- \
//!     --capture-dir /tmp/frames/ibl --capture-frames 150
//! ```
...
fn main() {
    ...
    app.insert_resource(SceneLights {
        directional: DirectionalLight {
            ...
            intensity: 0.3,  // very dim — let IBL dominate
        },
        point_lights: vec![],  // no point lights
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: IBL Lighting").with_size(640, 480);

    event_loop.run_app(&mut DemoIblApp {
...
impl DemoIblApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo IBL initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoIblApp {
