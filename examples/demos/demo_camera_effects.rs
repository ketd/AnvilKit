//! # Demo Camera Effects — 相机效果演示
//!
//! 相机抖动 + 头部晃动效果，用于演示相机特效。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_camera_effects -- \
//!     --capture-dir /tmp/frames/camera_effects --capture-frames 120
//! ```
...
fn main() {
    ...
    app.add_plugins(RenderPlugin::new().with_window_config(
        WindowConfig::new()
            .with_title("Demo: Camera Effects")
            .with_size(640, 480),
    ));
    ...
    let config = WindowConfig::new().with_title("Demo: Camera Effects").with_size(640, 480);

    event_loop.run_app(&mut DemoCameraEffectsApp {
...
impl DemoCameraEffectsApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo camera effects initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoCameraEffectsApp {
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        ...
        if let Some(ft) = self.app.world.get_resource::<FrameTime>() {
            ...
            // Camera shake: periodic impulses
            let shake_decay = (-(t % 3.0) * 5.0).exp();
            let shake_x = (t * 37.0).sin() * shake_decay * 0.15;
            let shake_y = (t * 53.0).sin() * shake_decay * 0.1;
            // Head bob
            let bob = (t * 4.0).sin() * 0.03;
            let eye = glam::Vec3::new(
                angle.sin() * radius + shake_x,
                height + bob + shake_y,
                angle.cos() * radius,
            );
