//! # Demo Color Grading — 色彩分级演示
//!
//! 动态变化的灯光颜色和强度，用于演示色彩分级效果。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_color_grading -- \
//!     --capture-dir /tmp/frames/color_grading --capture-frames 120
//! ```
...
fn main() {
    ...
    let config = WindowConfig::new().with_title("Demo: Color Grading").with_size(640, 480);

    event_loop.run_app(&mut DemoColorGradingApp {
...
impl DemoColorGradingApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo color grading initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoColorGradingApp {
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        ...
        // Animate light color: warm → cool → neutral cycle
        if let Some(ft) = self.app.world().get_resource::<FrameTime>() {
            let t = ft.0.elapsed().as_secs_f32();
            if let Some(mut lights) = self.app.world_mut().get_resource_mut::<SceneLights>() {
                let warm = (t * 0.3).sin() * 0.5 + 0.5;
                lights.directional.color = glam::Vec3::new(
                    0.6 + warm * 0.4,
                    0.7 + (1.0 - warm) * 0.1,
                    0.5 + (1.0 - warm) * 0.5,
                );
                lights.directional.intensity = 3.0 + warm * 3.0;
            }
        }
        self.app.update();
