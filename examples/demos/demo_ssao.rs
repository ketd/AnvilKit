//! # Demo SSAO — Cornell Box 场景展示环境光遮蔽
//!
//! 用 5 个 box 搭建 Cornell Box，演示 SSAO 在漫反射表面上的效果。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_ssao -- \
//!     --capture-dir /tmp/frames/ssao --capture-frames 120
//! ```
...
fn main() {
    ...
    app.insert_resource(SceneLights {
        ...
        point_lights: vec![
            PointLight { position: glam::Vec3::new(0.0, 1.5, -1.0), color: glam::Vec3::new(1.0, 1.0, 1.0), intensity: 10.0, range: 10.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: SSAO").with_size(640, 480);

    event_loop.run_app(&mut DemoSsaoApp {
...
impl DemoSsaoApp {
    fn init_scene(&mut self) {
        ...
        // White fallback textures for procedural geometry (no texture maps)
        ...
        // Helper closure to create a material bind group (each entity needs its own)
        let make_mat_bg = |label: &str| {
        ...
        let boxes = [
            BoxDef { pos: [0.0, 0.0, 2.0],    scale: [4.0, 4.0, 0.2], label: "BackWall" },
            BoxDef { pos: [-2.0, 0.0, 0.0],   scale: [0.2, 4.0, 4.0], label: "LeftWall" },
            BoxDef { pos: [2.0, 0.0, 0.0],    scale: [0.2, 4.0, 4.0], label: "RightWall" },
            BoxDef { pos: [0.0, -2.0, 0.0],   scale: [4.0, 0.2, 4.0], label: "Floor" },
            BoxDef { pos: [0.5, -1.5, 0.5],   scale: [1.0, 1.0, 1.0], label: "SmallCube" },
        ];

        let box_handles: Vec<_> = {
            ...
            boxes.iter().map(|b| {
                let mesh_data = generate_box([b.scale[0] * 0.5, b.scale[1] * 0.5, b.scale[2] * 0.5]);
                let (verts, indices) = mesh_data_to_pbr(&mesh_data);
                let mesh_h = assets.upload_mesh_u32(device, &verts, &indices, b.label);
                let bg = make_mat_bg(b.label);
                let mat_h = assets.create_material_with_pipeline(pipeline_handle, bg);
                (mesh_h, mat_h, b.pos)
            }).collect()
        };

        for (mesh_h, mat_h, pos) in box_handles {
        ...
        let eye = glam::Vec3::new(0.0, 0.5, -5.0);
        ...
        println!("Demo SSAO initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoSsaoApp {
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        // Orbit camera around the Cornell box
