//! # Demo NavMesh — Agent Pathfinding Visualization
//!
//! Spawns a 5x5 floor tile grid with obstacles and an animated agent sphere
//! that follows a set of waypoints. Full PBR pipeline with CSM shadows.
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_navmesh -- \
//!     --capture-dir /tmp/frames/navmesh --capture-frames 240
//! ```
...
/// Marker component for the agent entity so we can find it in about_to_wait.
#[derive(Component)]
struct AgentMarker;
...
/// Waypoints for the agent path.
const WAYPOINTS: [[f32; 3]; 5] = [
    [-2.0, 0.2, -2.0],
    [-2.0, 0.2,  0.0],
    [ 0.0, 0.2,  0.0],
    [ 2.0, 0.2,  0.0],
...
/// Total time to traverse all waypoints (seconds), then loops.
const PATH_DURATION: f32 = 8.0;

fn agent_position(t: f32) -> glam::Vec3 {
    let segment_count = WAYPOINTS.len() - 1;
    let looped_t = t % PATH_DURATION;
    let frac = looped_t / PATH_DURATION;
    let seg_f = frac * segment_count as f32;
    let seg = (seg_f as usize).min(segment_count - 1);
    let local = seg_f - seg as f32;

    let a = glam::Vec3::from(WAYPOINTS[seg]);
    let b = glam::Vec3::from(WAYPOINTS[seg + 1]);
    a + (b - a) * local
}
...
fn main() {
    ...
    app.add_plugins(RenderPlugin::new().with_window_config(
        WindowConfig::new()
            .with_title("Demo: NavMesh")
            .with_size(640, 480),
    ));
    app.insert_resource(FrameTime(Instant::now()));
    app.insert_resource(SceneLights {
        directional: DirectionalLight {
            direction: glam::Vec3::new(-0.3, -0.9, 0.2).normalize(),
            color: glam::Vec3::new(1.0, 0.97, 0.92),
            intensity: 4.5,
        },
        point_lights: vec![
            PointLight { position: glam::Vec3::new(0.0, 4.0, 0.0), color: glam::Vec3::new(1.0, 0.9, 0.7), intensity: 8.0, range: 12.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: NavMesh").with_size(640, 480);

    event_loop.run_app(&mut DemoNavmeshApp {
...
impl DemoNavmeshApp {
    fn init_scene(&mut self) {
        ...
        // Shared textures for procedural materials
        ...
        let mat_entries = [tex_entry(0), tex_entry(1), tex_entry(2), tex_entry(3), tex_entry(4),
        ...
        // Second compatible layout for creating bind groups after pipeline consumes mat_bgl
        let mat_bgl2 = device.device().create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Mat BGL2"),
            entries: &mat_entries,
        });
        let make_mat_bg = |bc: &wgpu::TextureView, label: &str| -> wgpu::BindGroup {
            device.device().create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some(label), layout: &mat_bgl2,
                entries: &[
        ...
        // Upload meshes
        let floor_data = generate_box([0.45, 0.05, 0.45]);
        let (fv, fi) = mesh_data_to_pbr(&floor_data);

        let obstacle_data = generate_box([0.3, 0.5, 0.3]);
        let (ov, oi) = mesh_data_to_pbr(&obstacle_data);

        let agent_data = generate_sphere(0.15, 12, 8);
        let (av, ai) = mesh_data_to_pbr(&agent_data);

        // Upload meshes and create materials, collecting handles before spawning
        let (floor_mesh, floor_mat, obstacle_mesh, obstacle_mat, agent_mesh, agent_mat) = {
            let mut assets = self.app.world.resource_mut::<RenderAssets>();
            let floor_mesh = assets.upload_mesh_u32(device, &fv, &fi, "Floor Tile");
            let obstacle_mesh = assets.upload_mesh_u32(device, &ov, &oi, "Obstacle");
            let agent_mesh = assets.upload_mesh_u32(device, &av, &ai, "Agent Sphere");
            let pipeline_handle = assets.register_pipeline(pipeline.into_pipeline());

            // Materials
            let floor_bc = make_tex(&[180, 180, 170, 255], "Floor BC", true);
            let floor_mat = assets.create_material_with_pipeline(pipeline_handle, make_mat_bg(&floor_bc, "Floor Mat"));

            let obstacle_bc = make_tex(&[140, 60, 60, 255], "Obstacle BC", true);
            let obstacle_mat = assets.create_material_with_pipeline(pipeline_handle, make_mat_bg(&obstacle_bc, "Obstacle Mat"));

            let agent_bc = make_tex(&[255, 180, 50, 255], "Agent BC", true);
            let agent_mat = assets.create_material_with_pipeline(pipeline_handle, make_mat_bg(&agent_bc, "Agent Mat"));

            (floor_mesh, floor_mat, obstacle_mesh, obstacle_mat, agent_mesh, agent_mat)
        }; // assets dropped here

        // Spawn floor tiles: 5x5 grid
        for x in 0..5u32 {
            for z in 0..5u32 {
                let px = x as f32 - 2.0;
                let pz = z as f32 - 2.0;
                self.app.world.spawn((
                    floor_mesh, floor_mat,
                    MaterialParams {
                        metallic: 0.0,
                        roughness: 0.9,
                        normal_scale: 1.0,
                    ...
                    GlobalTransform(glam::Mat4::from_translation(glam::Vec3::new(px, 0.0, pz))),
                ));
            }
        }

        // Spawn obstacles
        let obstacle_positions: [[f32; 3]; 3] = [
            [ 1.0, 0.25,  1.0],
            [-1.0, 0.25, -1.0],
            [ 0.0, 0.25,  2.0],
        ];
        for pos in &obstacle_positions {
            self.app.world.spawn((
                ...
                GlobalTransform(glam::Mat4::from_translation(glam::Vec3::new(pos[0], pos[1], pos[2]))),
            ));
        }

        // Spawn agent sphere at initial waypoint
        let start = agent_position(0.0);
        self.app.world.spawn((
            agent_mesh, agent_mat,
            MaterialParams {
                metallic: 0.1,
                roughness: 0.3,
                normal_scale: 1.0,
            ...
            GlobalTransform(glam::Mat4::from_translation(start)),
            AgentMarker,
        ));

        // Camera (top-down-ish)
        let eye = glam::Vec3::new(0.0, 5.0, -6.0);
        ...
        println!("Demo navmesh initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoNavmeshApp {
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        // Animate agent along waypoints
        if let Some(ft) = self.app.world.get_resource::<FrameTime>() {
            let t = ft.0.elapsed().as_secs_f32();
            let pos = agent_position(t);

            // Update agent transform
            for (_marker, mut transform, mut gt) in self.app.world.query::<(&AgentMarker, &mut Transform, &mut GlobalTransform)>().iter_mut(&mut self.app.world) {
                transform.translation = pos;
                gt.0 = glam::Mat4::from_translation(transform.translation);
            }

            // Orbit camera (top-down-ish, slow)
            let radius = 6.0;
            let height = 5.0;
            let speed = 0.15;
            let angle = t * speed;
