//! # Demo Showcase — 可配置 PBR 场景 + 自动帧捕获
//!
//! 基于 showcase.rs 的自动录帧版本。通过命令行参数控制效果和输出。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_showcase -- \
...
fn main() {
    ...
    let config = WindowConfig::new().with_title("Demo Showcase").with_size(640, 480);

    event_loop.run_app(&mut DemoShowcaseApp {
...
impl DemoShowcaseApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo showcase initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoShowcaseApp {
