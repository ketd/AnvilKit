//! # Demo Motion Blur — 快速旋转物体展示运动模糊概念
//!
//! 固定摄像机，DamagedHelmet 高速自转，展示运动模糊视觉效果。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_motion_blur -- \
//!     --capture-dir /tmp/frames/motion_blur --capture-frames 120
//! ```
...
fn main() {
    ...
    let config = WindowConfig::new().with_title("Demo: Motion Blur").with_size(640, 480);

    event_loop.run_app(&mut DemoMotionBlurApp {
...
impl DemoMotionBlurApp {
    fn init_scene(&mut self) {
        ...
        // Camera — static position looking at the helmet
        let eye = glam::Vec3::new(3.5, 0.5, 0.0);
        ...
        println!("Demo Motion Blur initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoMotionBlurApp {
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        // Static camera — rotate the object instead
        if let Some(ft) = self.app.world().get_resource::<FrameTime>() {
            let t = ft.0.elapsed().as_secs_f32();

            // Rotate the helmet quickly (non-camera entities = those without CameraComponent)
            for (mut transform, mut gt) in self.app.world_mut().query_filtered::<(&mut Transform, &mut GlobalTransform), Without<CameraComponent>>().iter_mut(self.app.world_mut()) {
                transform.rotation = glam::Quat::from_rotation_y(t * 3.0);
                gt.0 = glam::Mat4::from_quat(transform.rotation);
            }
        }
        self.app.update();
