//! # Demo Particles — 粒子系统演示
//!
//! 多彩点光源 + 快速轨道相机 + Bloom 效果，用于演示粒子系统。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_particles -- \
//!     --capture-dir /tmp/frames/particles --capture-frames 120
//! ```
...
/// Marker component for firefly particle entities, stores the orbit index.
#[derive(Component)]
struct Firefly(usize);

/// Convert procedural MeshData to PbrVertex + u32 indices.
...
fn main() {
    ...
    app.insert_resource(SceneLights {
        ...
        point_lights: vec![
            PointLight { position: glam::Vec3::new(2.0, 1.0, -1.0), color: glam::Vec3::new(1.0, 0.2, 0.2), intensity: 20.0, range: 10.0 },
            PointLight { position: glam::Vec3::new(-2.0, 1.0, 1.0), color: glam::Vec3::new(0.2, 0.3, 1.0), intensity: 18.0, range: 10.0 },
            PointLight { position: glam::Vec3::new(0.0, 2.0, -2.0), color: glam::Vec3::new(0.2, 1.0, 0.3), intensity: 15.0, range: 10.0 },
            PointLight { position: glam::Vec3::new(1.0, -0.5, 2.0), color: glam::Vec3::new(1.0, 1.0, 0.2), intensity: 17.0, range: 10.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: Particles").with_size(640, 480);

    event_loop.run_app(&mut DemoParticlesApp {
...
impl DemoParticlesApp {
    fn init_scene(&mut self) {
        ...
        // Fallback textures for particle spheres
        let white_view = create_texture(device, 1, 1, &[255, 255, 255, 255], "White").1;
        let normal_view = create_texture_linear(device, 1, 1, &[128, 128, 255, 255], "FlatNormal").1;
        ...
        let mat_bgl = device.device().create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Mat BGL"), entries: &mat_entries,
        });
        // Second compatible layout for creating particle bind groups after pipeline consumes mat_bgl
        let mat_bgl2 = device.device().create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("Mat BGL2"), entries: &mat_entries,
        });
        let mat_bg = device.device().create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Helmet Mat BG"), layout: &mat_bgl2,
            entries: &[
        ...
        // Upload helmet mesh and particle sphere, collect all handles before spawning
        let sphere_data = generate_sphere(0.05, 8, 6);
        ...
        let (mesh_h, mat_h, sphere_mesh_h, particle_handles) = {
            ...
            let sphere_mesh_h = assets.upload_mesh_u32(device, &sv, &si, "Firefly Sphere");

            // Create 12 firefly particle handles
            let mut particle_handles = Vec::with_capacity(12);
            for i in 0..12usize {
                let particle_bg = device.device().create_bind_group(&wgpu::BindGroupDescriptor {
                    label: Some("Particle Mat BG"), layout: &mat_bgl2,
                    entries: &[
                ...
                let particle_mat_h = assets.create_material_with_pipeline(pipeline_handle, particle_bg);
                let angle = std::f32::consts::TAU * i as f32 / 12.0;
                let radius = 1.5 + i as f32 * 0.1;
                let x = angle.sin() * radius;
                let z = angle.cos() * radius;
                let y = (i as f32 * 0.5).sin() * 0.5;
                particle_handles.push((particle_mat_h, x, y, z, i));
            }

            (mesh_h, mat_h, sphere_mesh_h, particle_handles)
        }; // assets dropped here
        ...
        for (particle_mat_h, x, y, z, i) in particle_handles {
            self.app.world.spawn((
                ...
                MaterialParams {
                    ...
                    roughness: 0.5,
                    normal_scale: 1.0,
                    emissive_factor: [2.0, 1.5, 0.5],
                },
                Transform::from_xyz(x, y, z),
                GlobalTransform(glam::Mat4::from_translation(glam::Vec3::new(x, y, z))),
                Firefly(i),
            ));
        }
        ...
        println!("Demo Particles initialized (capture: {})", self.capture.recording);
    }
    ...
    fn render_frame(&mut self) {
        ...
        for (i, cmd) in dl.commands.iter().enumerate() {
            ...
            {
                let cl = if i == 0 { wgpu::LoadOp::Clear(wgpu::Color { r: 0.01, g: 0.01, b: 0.02, a: 1.0 }) } else { wgpu::LoadOp::Load };
        ...
        // Bloom with low threshold to make emissive particles glow
...
impl ApplicationHandler for DemoParticlesApp {
