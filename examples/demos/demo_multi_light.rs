//! # Demo Multi-Light — Multiple colorful point lights orbiting a PBR model
//!
//! Based on demo_showcase.rs, this demo features 6 colorful point lights
//! arranged in a circle around the helmet, animating their positions over time.
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_multi_light -- \
//!     --capture-dir /tmp/frames/multi_light --capture-frames 120
//! ```
...
fn main() {
    ...
    app.insert_resource(SceneLights {
        ...
        point_lights: vec![
            // Red
            PointLight { position: glam::Vec3::new(3.0, 1.0, 0.0), color: glam::Vec3::new(1.0, 0.2, 0.2), intensity: 15.0, range: 8.0 },
            // Green
            PointLight { position: glam::Vec3::new(-3.0, 1.0, 0.0), color: glam::Vec3::new(0.2, 1.0, 0.2), intensity: 15.0, range: 8.0 },
            // Blue
            PointLight { position: glam::Vec3::new(0.0, 1.0, 3.0), color: glam::Vec3::new(0.2, 0.2, 1.0), intensity: 15.0, range: 8.0 },
            // Yellow
            PointLight { position: glam::Vec3::new(0.0, 1.0, -3.0), color: glam::Vec3::new(1.0, 1.0, 0.2), intensity: 15.0, range: 8.0 },
            // Cyan
            PointLight { position: glam::Vec3::new(2.0, 2.0, 2.0), color: glam::Vec3::new(0.2, 1.0, 1.0), intensity: 12.0, range: 8.0 },
            // Magenta
            PointLight { position: glam::Vec3::new(-2.0, 2.0, -2.0), color: glam::Vec3::new(1.0, 0.2, 1.0), intensity: 12.0, range: 8.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: Multi-Light").with_size(640, 480);

    event_loop.run_app(&mut DemoMultiLightApp {
...
impl DemoMultiLightApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo multi-light initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoMultiLightApp {
    ...
    fn about_to_wait(&mut self, el: &ActiveEventLoop) {
        if let Some(ft) = self.app.world().get_resource::<FrameTime>() {
            ...
            // Orbit camera (slower speed, slightly higher)
            let radius = 3.5;
            let height = 0.2;
            let speed = 0.3;
            ...
            // Animate point light positions: rotate each light around Y axis at different speeds
            // Each light orbits at its own angular speed with a unique phase offset.
            if let Some(mut lights) = self.app.world_mut().get_resource_mut::<SceneLights>() {
                // Initial radii and heights for each light (derived from starting positions)
                let light_configs: [(f32, f32, f32, f32); 6] = [
                    // (radius, height, angular_speed, phase_offset)
                    (3.0, 1.0, 0.8,  0.0),                          // Red
                    (3.0, 1.0, 0.8,  std::f32::consts::PI),         // Green (opposite side)
                    (3.0, 1.0, 0.6,  std::f32::consts::FRAC_PI_2),  // Blue
                    (3.0, 1.0, 0.6,  -std::f32::consts::FRAC_PI_2), // Yellow (opposite side)
                    (2.83, 2.0, 1.0, std::f32::consts::FRAC_PI_4),  // Cyan  (sqrt(2^2+2^2))
                    (2.83, 2.0, 1.0, -std::f32::consts::FRAC_PI_4 * 3.0), // Magenta
                ];

                for (i, light) in lights.point_lights.iter_mut().enumerate() {
                    if i >= light_configs.len() { break; }
                    let (r, h, angular_speed, phase) = light_configs[i];
                    let angle = t * angular_speed + phase;
                    light.position = glam::Vec3::new(
                        angle.cos() * r,
                        h + (t * angular_speed * 0.5 + phase).sin() * 0.3,
                        angle.sin() * r,
                    );
                }
            }
