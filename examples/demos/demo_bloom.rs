//! # Demo Bloom — 泛光后处理效果演示
//!
//! 展示 HDR Bloom 效果：高发光材质 + 低阈值 + 高强度泛光。
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_bloom -- \
//!     --capture-dir /tmp/frames/bloom --capture-frames 120
//! ```
...
fn main() {
    ...
    app.insert_resource(SceneLights {
        ...
        point_lights: vec![
            PointLight { position: glam::Vec3::new(1.0, 0.5, -1.0), color: glam::Vec3::new(1.0, 0.9, 0.7), intensity: 25.0, range: 5.0 },
            PointLight { position: glam::Vec3::new(3.0, 2.0, -2.0), color: glam::Vec3::new(1.0, 0.6, 0.3), intensity: 15.0, range: 10.0 },
            PointLight { position: glam::Vec3::new(-3.0, 1.0, -1.0), color: glam::Vec3::new(0.3, 0.5, 1.0), intensity: 10.0, range: 10.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: Bloom").with_size(640, 480);

    event_loop.run_app(&mut DemoBloomApp {
...
impl DemoBloomApp {
    fn init_scene(&mut self) {
        ...
        println!("Demo bloom initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoBloomApp {
