//! # Demo Physics — Domino Tower Pyramid
//!
//! Spawns 15 boxes in a pyramid arrangement with per-row color variations.
//! Full PBR pipeline with CSM shadows, HDR, bloom, and tonemap.
//!
//! ```bash
//! cargo run -p anvilkit-render --features capture --example demo_physics -- \
//!     --capture-dir /tmp/frames/physics --capture-frames 120
//! ```
...
fn main() {
    ...
    app.insert_resource(SceneLights {
        directional: DirectionalLight {
            ...
            color: glam::Vec3::new(1.0, 0.98, 0.90),
            intensity: 5.0,
        },
        point_lights: vec![
            PointLight { position: glam::Vec3::new(3.0, 3.0, -2.0), color: glam::Vec3::new(1.0, 0.8, 0.5), intensity: 10.0, range: 12.0 },
        ],
        spot_lights: vec![],
    });

    let event_loop = EventLoop::new().unwrap();
    let config = WindowConfig::new().with_title("Demo: Physics").with_size(640, 480);

    event_loop.run_app(&mut DemoPhysicsApp {
...
impl DemoPhysicsApp {
    fn init_scene(&mut self) {
        ...
        // Upload box mesh (shared by all 15 boxes)
        let box_data = generate_box([0.4, 0.4, 0.4]);
        let (bv, bi) = mesh_data_to_pbr(&box_data);

        // Per-row colors: slight warm-to-cool gradient
        let row_colors: [[u8; 4]; 5] = [
            [200, 100,  80, 255],  // row 0: warm red-orange
            [220, 160,  70, 255],  // row 1: golden
            [180, 200,  90, 255],  // row 2: yellow-green
            [100, 180, 200, 255],  // row 3: teal
            [120, 110, 210, 255],  // row 4: blue-violet
        ];

        // Pyramid layout: row i has (5 - i) boxes
        let rows: [(f32, usize); 5] = [
            (-1.0, 5),   // row 0
            (-0.2, 4),   // row 1
            ( 0.6, 3),   // row 2
            ( 1.4, 2),   // row 3
        ...
        // Upload mesh and create all material handles before spawning
        let (box_mesh, row_handles) = {
            let mut assets = self.app.world_mut().resource_mut::<RenderAssets>();
            let box_mesh = assets.upload_mesh_u32(device, &bv, &bi, "Domino Box");
            let pipeline_handle = assets.register_pipeline(pipeline.into_pipeline());

            let mut row_handles = Vec::new();
            for (row_idx, &(y, count)) in rows.iter().enumerate() {
                let bc_view = make_tex(&row_colors[row_idx], &format!("Row{} BC", row_idx), true);
                let mat_bg = make_mat_bg(&bc_view, &format!("Row{} Mat", row_idx));
                let mat_h = assets.create_material_with_pipeline(pipeline_handle, mat_bg);
                row_handles.push((mat_h, y, count));
            }

            (box_mesh, row_handles)
        }; // assets dropped here

        for (mat_h, y, count) in row_handles {
            let x_start = -((count as f32 - 1.0) * 0.5) * 1.0;
            for col in 0..count {
                let x = x_start + col as f32 * 1.0;
                self.app.world_mut().spawn((
                    ...
                    GlobalTransform(glam::Mat4::from_translation(glam::Vec3::new(x, y, 0.0))),
                ));
            }
        }

        // Camera
        let eye = glam::Vec3::new(0.0, 1.0, -6.0);
        ...
        println!("Demo physics initialized (capture: {})", self.capture.recording);
    }
...
impl ApplicationHandler for DemoPhysicsApp {
