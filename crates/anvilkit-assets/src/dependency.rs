...
pub struct DependencyGraph {
    ...
    reverse: HashMap<AssetId, HashSet<AssetId>>,
}

// Use a static empty set for returning references when key not found
static EMPTY: std::sync::LazyLock<HashSet<AssetId>> = std::sync::LazyLock::new(HashSet::new);
...
impl DependencyGraph {
    ...
    pub fn remove_dependency(&mut self, parent: AssetId, child: AssetId) {
    ...
    pub fn dependencies_of(&self, id: AssetId) -> &HashSet<AssetId> {
        self.deps.get(&id).unwrap_or(&EMPTY)
    }

    /// Assets that depend on this asset.
    pub fn dependents_of(&self, id: AssetId) -> &HashSet<AssetId> {
        self.reverse.get(&id).unwrap_or(&EMPTY)
    }

    /// Check if any other asset depends on this one.
    pub fn has_dependents(&self, id: AssetId) -> bool {
    ...
    pub fn remove_and_cascade(&mut self, id: AssetId) -> Vec<AssetId> {
        let children: Vec<AssetId> = self.deps.get(&id)
        ...
        while let Some(orphan) = queue.pop_front() {
            let grandchildren: Vec<AssetId> = self.deps.get(&orphan)
                .map(|s| s.iter().copied().collect())
                .unwrap_or_default();
...
mod tests {
    ...
    /// Helper to create an AssetId from a raw u64.
    fn id(n: u64) -> AssetId {
        AssetId::from_raw(n)
    }
    ...
    fn add_and_query() {
        ...
        g.add_dependency(id(1), id(2));
        g.add_dependency(id(1), id(3));
        assert!(g.dependencies_of(id(1)).contains(&id(2)));
        assert!(g.dependencies_of(id(1)).contains(&id(3)));
        assert!(g.dependents_of(id(2)).contains(&id(1)));
        assert!(g.has_dependents(id(2)));
        assert!(!g.has_dependents(id(1)));
    }
    ...
    fn remove_dependency_edge() {
        let mut g = DependencyGraph::new();
        g.add_dependency(id(1), id(2));
        g.remove_dependency(id(1), id(2));
        assert!(g.dependencies_of(id(1)).is_empty());
        assert!(!g.has_dependents(id(2)));
        assert!(g.is_empty());
    }
    ...
    fn cascade_simple() {
        ...
        let mut g = DependencyGraph::new();
        g.add_dependency(id(1), id(2));
        g.add_dependency(id(2), id(3));
        let cascaded = g.remove_and_cascade(id(1));
        // B has no other parents -> cascaded, then C has no parents -> cascaded
        assert!(cascaded.contains(&id(2)));
        assert!(cascaded.contains(&id(3)));
        assert!(g.is_empty());
    }
    ...
    fn shared_dep_no_cascade() {
        ...
        assert!(g.has_dependents(id(3))); // B still depends on C
    }
    ...
    fn diamond_cascade() {
        ...
        g.add_dependency(id(1), id(2));
        g.add_dependency(id(1), id(3));
        g.add_dependency(id(2), id(4));
        g.add_dependency(id(3), id(4));
        let cascaded = g.remove_and_cascade(id(1));
        assert!(cascaded.contains(&id(2)));
        assert!(cascaded.contains(&id(3)));
        assert!(cascaded.contains(&id(4)));
        assert!(g.is_empty());
    }
    ...
    fn diamond_partial_cascade() {
        ...
        g.add_dependency(id(2), id(4));
        g.add_dependency(id(3), id(4));
        g.add_dependency(id(5), id(4));
        let cascaded = g.remove_and_cascade(id(1));
        assert!(cascaded.contains(&id(2)));
        assert!(cascaded.contains(&id(3)));
        assert!(!cascaded.contains(&id(4))); // E still holds D
        assert!(g.has_dependents(id(4)));
    }
    ...
    fn empty_graph() {
        ...
        assert!(g.is_empty());
        assert_eq!(g.len(), 0);
        assert!(g.dependencies_of(id(999)).is_empty());
        assert!(g.dependents_of(id(999)).is_empty());
    }
